...
docker build --build-arg TIMESTAMP="${TIMESTAMP}_${GITHASH}" -t ${IMAGE}:build .

echo " "
echo "Running the build container to load the data"

# start it with the proper conf file
did=`docker run -d -p 8889:8080 ${IMAGE}:build`
status=$?
...
echo " "
echo "docker commit $did ${IMAGE}:${TIMESTAMP}_${GITHASH}_models "
docker commit $did ${IMAGE}:${TIMESTAMP}_${GITHASH}_models
echo " "
...
ls -1 *txt | xargs -P 1 -n 1 -I, bash -c 'echo "  $(date) Loading measurments file ,";python3 ../InsertMeasurements.py ,'

echo " "
rangeCount=`curl -s -G -H 'Accept: application/xml' "${url_blazegraph}sparql" --data-urlencode ESTCARD | sed 's/.*rangeCount=\"\([0-9]*\)\".*/\1/'`
echo "Finished loading blazegraph measurements ($rangeCount)"
echo " "
echo "docker commit $did ${IMAGE}:${TIMESTAMP}_${GITHASH}_measurements "
docker commit $did ${IMAGE}:${TIMESTAMP}_${GITHASH}_measurements
